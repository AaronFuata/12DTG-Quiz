...
def test():
    with open('quiz.csv', mode='r') as QUIZCSV:
        ...
        if correct / questions >= 0.5:
            ...
            if score > highscore:
                print("That beats your previous highscore of " +
                      str(highscore) + ".")
                highscore = score
                save = 0
            ...
            print(
                "Quiz FAILED! Please go over definitions and try again later.")
...
def menu():
    ...
    while option != 0:
        print(question)
        question = "Please select an option below:"
        option = input("0 - quit, 1 - test, 2 - learn: ")
        if option == "1":
            test()
        elif option == "2":
            learn()
